...
label check_playermap_new:
    ...
    elif Char_Data["hero"]["map"] == Map_Data["city"]["name"]:
        call goto_map
        scene bg_map at Zoom_X(0.5)
        show screen nav_map

    elif Char_Data["hero"]["map"] == Map_Data["home"]["name"] + " - " + Map_Data["home"]["map"]["facade"]["name"]:
...
label wolfitdm_check_playermap:
    ...
    elif Char_Data["hero"]["map"] in wolfitdm_get_maps():
       $ wolfitdm_original_player_map = Char_Data["hero"]["map"]
       $ store.wolfitdm_original_player_map = Char_Data["hero"]["map"]
    else:
